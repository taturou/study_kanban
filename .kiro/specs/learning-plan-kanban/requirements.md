...
| タスクダイアログ | TDialog | UI（Dialog） | Task の作成・編集を行うダイアログ（個別/一括モード切替）。Task の全属性を入力・閲覧できる。 |
...
11. Where 学習者が予定（非学習を含む）を登録する, the LPK shall カレンダーに表示して学習可能時間調整の参考にする（学習可能時間には自動反映しない）。
