...
5. The LPK shall 設定画面で各ステータスの表示文字列を設定可能とし、追加・削除・並べ替えは不可とする。設定画面ではアプリのバージョンとアップデート状態も確認できる。
...
8. When 学習者がBacklog セルのプラス操作を行う, the LPK shall タスク作成ダイアログを開き、個別追加モード（詳細入力）と一括追加モード（複数行テキストで1行=1タスク）を選択できるようにする。Backlog の教科列から起動するため教科入力は不要とする。
