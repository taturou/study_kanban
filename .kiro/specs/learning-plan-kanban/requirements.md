...
| 設定パネル | SettingsPanel | UI（Panel） | ステータス表示名や言語、バージョン/アップデート状態などを扱う設定用パネル。 |
...
### Requirement 7: ローカライゼーション
**Objective:** 日本人以外のユーザーとして、言語を切り替えて表示したい。アクセシビリティ（WCAG 2.1 AA 準拠、TDialog 以外のキーボードナビゲーション、ARIA 属性）は対象外とし、今後の拡張要件として `.kiro/specs/todo.md` に記載する。

#### Acceptance Criteria
1. The LPK shall UI 言語を日本語と英語で切り替えることを許可する。
2. Where ユーザーが言語を選択する, the LPK shall その設定をローカルに保存し、次回以降の訪問時に適用する。また `settings.json` にも記録し、同期およびバックアップ/リストアの対象とする。
