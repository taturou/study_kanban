...
- 構造: `AppShell[AppBar]` と `KanbanView[Header][Container]` を分離し、`Container` のみを縦横にスクロール可能にする（AppBar と Kanban Header は固定）。
- `Container` 内ではステータス行を上に、教科列を左にピン留めして表示し、TCard 領域は縦横にスクロールする。
